...
class MEAD(data.Dataset):
    ...
    def __init__(self, opt, which='source', phase='train'):
        ...
        if phase == 'train':
            self.selected_actors = os.listdir(self.root) if 'all' in opt.selected_actors else opt.selected_actors
        elif phase == 'val':
        ...
        for actor in self.selected_actors:
            actor_root = os.path.join(self.root, '{}_MEAD_data.pkl'.format(actor.split('_')[0]))
            assert os.path.isfile(actor_root), '%s is not a valid file' % actor_root

            data_actor = pickle.load(open(actor_root, "rb"))
            print(actor.split('_')[0],len(data_actor))
            videos.extend(data_actor)
        print(self.selected_actors)

        for v in videos:
            params, params_detail, emotion, actor = v
            params = torch.cat((params[:,0:1], params[:,3:]),1)
            ...
            if not f:
                ...
                self.seq_paths.extend(seqs)
                self.labels.extend([label]*len(seqs))
        self.seq_paths = np.stack(self.seq_paths,axis=0)
        ...
        if self.which == 'reference':
            ...
            self.seq_paths = np.array(self.seq_paths)[p]

            self.labels = np.array(self.labels)[p].tolist()

    def is_valid_root(self, root):
        ret = False
        for e in self.selected_emotions:
            if e == 'neutral':
                check = f'{e}/level_1'
            else:
                check = f'{e}/level_3'
    ...
    def __getitem__(self, index):
        ...
        sequence = torch.FloatTensor(self.seq_paths[index])
...
class affwild2(data.Dataset):
    ...
    def __getitem__(self, index):
        """Return one sequence and its corresponding labels."""

        paths = self.seq_paths[index]
        elif self.facial_model=='FLAME':
        params = []
        for pth in paths:
