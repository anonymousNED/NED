...
from test import geometric_median, get_style_vectors, generate_exp_coeffs
from manipulator.util.visualization import generate_mesh_FLAME
...
def compute_g_loss(nets, opt, x_real, y_org, y_trg, z_trg=None, x_ref=None):
    ...
    loss_mouth_f = torch.mean(torch.mean(v_real * v_fake, dim=1) * torch.rsqrt(torch.mean(v_real ** 2, dim=1)) * torch.rsqrt(torch.mean(v_fake ** 2, dim=1)))   # PCC
    ...
    loss_mouth_b = torch.mean(torch.mean(v_fake * v_rec, dim=1) * torch.rsqrt(torch.mean(v_fake ** 2, dim=1)) * torch.rsqrt(torch.mean(v_rec ** 2, dim=1)))  # PCC
...
if __name__ == '__main__':
    ...
    print('Using {} dataset ...'.format(opt.database))
    ...
    for epoch in range(0 if opt.finetune else opt.resume_epoch, opt.niter):
        ...
        shape, shape_detail_images, tex_images, tex_detail_images = generate_mesh_FLAME(exp_coeffs_ground, opt)
        ...
        for k, e in enumerate(exp_coeffs):
            shape, shape_detail_images, tex_images, tex_detail_images = generate_mesh_FLAME(e, opt)
        ...
        for k, e in enumerate(exp_coeffs):
            shape, shape_detail_images, tex_images, tex_detail_images = generate_mesh_FLAME(e, opt)
            logger.log_mesh(shape, 'shape/Reference {}'.format(k), epoch)
            logger.log_mesh(shape_detail_images, 'shape_detail/Reference {}'.format(k), epoch)
            logger.log_mesh(tex_images, 'tex_images/Reference {}'.format(k), epoch)
            logger.log_mesh(tex_detail_images, 'tex_detail/Reference {}'.format(k), epoch)
